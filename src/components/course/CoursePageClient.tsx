...
export default function CoursePageClient({ courseId }: { courseId: string }) {
  ...
  React.useEffect(() => {
    async function loadData() {
      ...
      // Fetch user progress and check permissions if logged in
      if (session?.user) {
        ...
        // If not logged in, they are blocked (they will see the Auth lock screen)
        setHasAccess(false);
      }
                ...
                return (
                  ...
                  >
                    <button 
                      ...
                      className="block text-left w-full bg-white group p-8 rounded-[40px] border border-transparent shadow-sm hover:shadow-2xl hover:shadow-orange-100/30 transition-all duration-500"
                    >
